...
- `uploads[].logs`: An array of log entries emitted by the ingestion job. Each entry has `timestamp` (ISO 8601 UTC string), `type` (one of `info`, `warning`, `error`), and `message`.
